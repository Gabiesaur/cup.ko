    ...
    return (
        ...
        >
            ...
            <div className='flex flex-col items-center z-10 w-full max-w-[650px]'>
                {/* Main Pink Container */}
                <div className='relative w-full bg-[#cc8386] rounded-[30px] md:rounded-[40px] flex flex-col justify-start items-start pt-14 px-6 md:px-12 pb-12 gap-5 shadow-lg'>

                    {/* Header Badge / Dropdown Container */}
                    <div className='absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[85%] md:w-[90%] lg:w-[500px] z-50 flex flex-col items-center'>

                        {/* Main Button */}
                        <button
                            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                            className='h-16 md:h-20 w-full bg-[#cc8386] rounded-[40px] flex flex-row justify-center items-center gap-3 md:gap-4 cursor-pointer hover:opacity-95 transition-opacity shadow-md'
                        >
                            <img src={cart} alt="cart" className='h-8 w-8 md:h-10 md:w-10' />
                            <p className='text-[#f8cc1b] text-3xl lg:text-4xl font-bold [text-shadow:2px_3px_2px_#a8606c]' style={{ fontFamily: "Opun Mai Bold Italic" }}>
                                {mode}
                            </p>
                            <span className={`text-[#f8cc1b] text-2xl md:text-3xl transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`}>
                                ▼
                            </span>
                        </button>
                        ...
                        {/* Dropdown Menu */}
                        {isDropdownOpen && (
                            <div className='absolute top-20 md:top-24 w-[90%] bg-[#cc8386] rounded-[20px] flex flex-col overflow-hidden shadow-xl border-2 border-[#f8cc1b]/20'>
                                {['physical store', 'delivery', 'reservation'].map((option) => (
                                    <button
                                        key={option}
                                        onClick={() => handleModeSelect(option as BuyingMode)}
                                        className={`py-3 px-6 text-[#f8cc1b] text-xl md:text-3xl font-bold hover:bg-[#b06f72] transition-colors ${mode === option ? 'bg-[#b06f72]' : ''}`}
                    ...
                    <div className="w-full flex flex-col gap-2 relative z-0">
                        <label className="text-[#873641] text-xl md:text-2xl font-bold" style={{ fontFamily: "'Patrick Hand', cursive" }}>
                            Name:
                        </label>
                        <input
                            type="text"
                    ...
                    <div className="w-full flex flex-col gap-3 relative z-0">
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className={`flex-1 py-3 md:py-4 px-4 md:px-6 rounded-full text-base md:text-xl font-bold transition-colors ${paymentMode === 'gcash'
                            ...
                            <button
                                ...
                                className={`flex-1 py-3 md:py-4 px-4 md:px-6 rounded-full text-base md:text-xl font-bold transition-colors ${paymentMode === 'cash'
                                    ? 'bg-[#f8cc1b] text-[#873641]'
                                    : 'bg-[#fce18d] text-[#873641] hover:bg-[#f8cc1b]'
                                    }`}
                                style={{ fontFamily: "'Patrick Hand', cursive" }}
                    ...
                    {mode === 'delivery' && (
                        <div className="w-full flex flex-col gap-2 relative z-0">
                            ...
                            <input
                                ...
                                style={{ fontFamily: "'Patrick Hand', cursive" }}
                            />
                            <ul className="list-disc pl-6 text-[#873641] text-xs md:text-sm leading-tight pr-4 mt-1 opacity-80" style={{ fontFamily: "sans-serif" }}>
                    ...
                    {/* Next Button */}
                    <button
                        className="absolute -bottom-6 -right-4 md:-right-6 bg-[#fce18d] text-[#e1a0aa] text-2xl md:text-3xl hover:bg-[#e08a1d] transition-colors rounded-[30px] px-10 md:px-14 py-3 shadow-md z-0"
